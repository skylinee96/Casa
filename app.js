(() => {
  ...
  const render = () => {
    ...
    const taskMarkup = state.tasks.map(task => `<li><input type="checkbox" data-task="${task.id}" ${task.done?'checked':''}><label class="${task.done?'done':''}">${escape(task.name)}</label></li>`).join('');
    $('homeTasks').innerHTML=taskMarkup || `<li>${language==='pl'?'Brak zadań':'No tasks yet'}</li>`;
    $('listItems').innerHTML=state.tasks.map(task => `<li><input type="checkbox" data-task="${task.id}" ${task.done?'checked':''}><span class="event-name ${task.done?'done':''}">${escape(task.name)}</span></li>`).join('');
    const eventMarkup=state.events.sort((a,b)=>a.date.localeCompare(b.date)).map(event=>`<li><span class="event-date">${dateLabel(event.date)}</span><span><span class="event-name">${escape(event.name)}</span><span class="event-sub">${language==='pl'?'Wspólny plan':'Shared plan'}</span></span></li>`).join('');
    ...
    $('memberName').value=state.name; $('householdButton').textContent=(state.name || 'M').slice(0,1).toUpperCase();
  };
  ...
  $('eventForm').addEventListener('submit',event=>{event.preventDefault();const data=new FormData(event.target);state.events.push({id:crypto.randomUUID(),name:data.get('name'),date:data.get('date')});event.target.closest('dialog').close();event.target.reset();render();persist();});
  ...
  $('saveName').addEventListener('click',()=>{state.name=$('memberName').value.trim() || 'Marta';render();persist();});
